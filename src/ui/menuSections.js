...
const DEFAULT_OPEN_SECTION_BY_MODE = {
  viewer: "viewport",
  debug: "viewport",
};

function renderSection(section, context) {
  const { menuMode } = context;
  const isVisible = section.modes.includes(menuMode);
  const isOpen = isVisible && DEFAULT_OPEN_SECTION_BY_MODE[menuMode] === section.id;
  const panelId = `menu-section-panel-${section.id}`;

  return `
    <section
      class="menu-section${isOpen ? " is-open" : ""}"
      data-menu-section="${section.id}"
      data-menu-section-modes="${section.modes.join(" ")}"
      ${isVisible ? "" : "hidden"}
    >
      <button
        type="button"
        class="menu-section-toggle"
        data-menu-section-toggle
      ...
      <div
        class="menu-section-panel"
        id="${panelId}"
        data-menu-section-panel
        ${isOpen ? "" : "hidden"}
      >
...
export function buildMenuSectionsMarkup(context) {
